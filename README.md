...
# Optional: bounded read-only advisory analyst (Slice 15C2). Fake mode is the
# default and needs no credentials. Live requires ATLAS_ADVISORY_MODE=live,
# openai / gpt-4o-mini / temperature 0, and a LangSmith key.
# Successful stdout is one JSON envelope; logs go to stderr.
# uv run python -m atlas.advisor --research-job-id <id>
```
