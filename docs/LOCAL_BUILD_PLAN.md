...
**Status:** Current (working branch `milestone-14-backend-containers`). Slice 14A is locally verified (not yet Complete — no PR opened). Slices 14B and 14C are Pending.
...
Full Docker Compose topology (API, worker, outbox relay, Kafka consumer, plus one-shot `db-migrate` and `kafka-topic-init` jobs) on one internal network, a new `python -m atlas.outbox.topic_admin` executable entry point (no topic-creation logic duplicated — it calls the existing `ensure_topic_exists`/`verify_topic_partitioning` functions), and local end-to-end/smoke verification against the real PostgreSQL/Redis/Kafka dependencies. Compose must not override the image's `ENTRYPOINT` (Tini already provides the portable init boundary proven in Slice 14A) and should not additionally set `init: true` unless testing later proves a concrete need for double-init — none was found in Slice 14A.

### Slice 14C — GitHub Actions build, runtime, and security gates

A separate `.github/workflows/containers.yml` workflow (required check, alongside the unchanged `.github/workflows/ci.yml`): builds the shared image once, runs the full Compose topology, executes smoke tests against the running containers, generates an SBOM (Syft) and scans for vulnerabilities (Trivy) against the exact tested image, and retains sanitized build/test/scan evidence as artifacts.
...
**Completion gate:** A clean checkout can build and run the backend platform (API, worker, outbox relay, Kafka consumer, and their PostgreSQL/Redis/Kafka dependencies) through Docker Compose with no host-installed Python tooling beyond Docker, and PR/`main` container CI (Slice 14C) passes.
