...
- Plan and draft are model-backed through Atlas `ResearchPlanner` / `ResearchDrafter` Protocols. Research uses governed `ResearchPlanExecutor` tools. Findings remain `list[str]` for Milestone 9 (richer evidence → Milestone 10).
...
- Checkpoint writes, Atlas audit writes, and ledger writes use separate connections/transactions and are not atomic together; after a crash they may briefly disagree. Checkpoints are the resume source of truth; audit and ledger rows are operational history.
...
These decisions cover the verified foundation through the Milestone 9 governed-tools slice. They do not imply RAG/evidence schemas, specialists, messaging, or cloud topology choices.
